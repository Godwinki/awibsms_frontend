...
import { X, Edit, Navigation, Square, Phone, FileText, Shield } from 'lucide-react';
import { shortcutService, Shortcut, UpdateShortcutData } from '@/lib/services/shortcut.service';
...
export function EditShortcutDialog({ isOpen, onClose, onSuccess, shortcut }: EditShortcutDialogProps) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<UpdateShortcutData>({});
  const [selectedRoles, setSelectedRoles] = useState<string[]>([]);
  const [actionDataJson, setActionDataJson] = useState('{}');

  // Initialize form data when shortcut changes
  useEffect(() => {
    if (shortcut) {
      setFormData({
        code: shortcut.originalCode,
        name: shortcut.name,
      ...
      // Set roles - handle both array and undefined cases
      const roles = Array.isArray(shortcut.actionData?.requiredRoles) 
        ? shortcut.actionData.requiredRoles 
        : [];
      setSelectedRoles(roles);
      
      // Set action data JSON
      setActionDataJson(JSON.stringify(shortcut.actionData || {}, null, 2));
    }
  }, [shortcut]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData: UpdateShortcutData = {
        ...formData,
        actionData: parsedActionData,
        requiredRoles: selectedRoles
      };

      await shortcutService.updateShortcut(shortcut.id, updateData);
  ...
  const handleActionTypeChange = (actionType: string) => {
    ...
    // Only update template if current JSON is empty or default
    if (actionDataJson === '{}' || actionDataJson.trim() === '') {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled={shortcut.isSystem}
                    required
                  />
                  {shortcut.isSystem && (
                    <p className="text-xs text-amber-600">System shortcuts cannot change codes</p>
                  )}
                </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading}>
              {loading ? 'Updating...' : 'Update Shortcut'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
