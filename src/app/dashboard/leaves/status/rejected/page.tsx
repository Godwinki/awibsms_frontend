...
export default function RejectedLeavesPage() {
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      setIsLoading(true);
      const all = await leaveService.getLeaveRequests({ status: "REJECTED" });
      setLeaves(all);
      setIsLoading(false);
    };
    fetchLeaves();
  ...
  return (
    <div className="space-y-6">
      ...
      <h2 className="text-2xl font-bold">Rejected Leave Requests</h2>
      ...
      <div className="rounded-md border">
        <LeavesTable leaves={filtered} isLoading={isLoading} emptyText="No rejected leave requests found." />
      </div>
    </div>
  );
}
