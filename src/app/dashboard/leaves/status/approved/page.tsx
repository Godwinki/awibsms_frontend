...
export default function ApprovedLeavesPage() {
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      setIsLoading(true);
      const all = await leaveService.getLeaveRequests({ status: "APPROVED" });
      setLeaves(all);
      setIsLoading(false);
    };
    fetchLeaves();
  ...
  return (
    <div className="space-y-6">
      ...
      <h2 className="text-2xl font-bold">Approved Leave Requests</h2>
      ...
      <div className="rounded-md border">
        <LeavesTable leaves={filtered} isLoading={isLoading} emptyText="No approved leave requests found." />
      </div>
    </div>
  );
}
